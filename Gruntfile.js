...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'VARIUST';
    var email     = grunt.option('email')  || 'info@variust.pl';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new imperihome name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/imperihome.png')) {
            fs.renameSync(__dirname + '/admin/imperihome.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/imperihome.html')) {
            fs.renameSync(__dirname + '/widgets/imperihome.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/imperihome/js/imperihome.js')) {
            fs.renameSync(__dirname + '/widgets/imperihome/js/imperihome.js', __dirname + '/widgets/imperihome/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/imperihome')) {
            fs.renameSync(__dirname + '/widgets/imperihome',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Imperihome/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Imperihome'
                        },
                        {
                            match: /VARIUST/g,
                            replacement: author
